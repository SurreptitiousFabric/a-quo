...
| [#2 Continuity and audit hardening](https://github.com/SurreptitiousFabric/a-quo/issues/2) | Core identity | **Implementing** (was Prototype complete) | Defined | Continuity supports an independently supplied head checkpoint; deterministic attack matrices cover prefixes, forks, omissions, duplicates, reordering, cross-persona splices, partial journal rewrites, and backup-event mutations; real abort tests cover both transaction sides; and schema v4 bounds and replays lifecycle state before authorization reads and mutations. Bounded production byte parsers plus pinned, synthetic-seed, sanitizer-backed campaigns cover continuity/recovery and backup parsing. Evidence-only backup v2 now preserves and internally reverifies supplied routine/recovery history without adopting authority. Live recovery commit/adoption, journaled current-head compromise, safe external pin/fork comparison, external witnessing, sustained fuzzing and broader leak analysis, and external security review remain open. |
