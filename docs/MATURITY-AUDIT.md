...
| [#7 Package A Quo safely for Omarchy](https://github.com/SurreptitiousFabric/a-quo/issues/7) | Omarchy | **Implementing** (was Design) | Defined | The shared [package and support contract](PACKAGING.md) fixes the three-binary layout, disabled per-user service, busless authority boundary, administrator/user ownership split, one complete pinned Omarchy 4 aarch64 journey, lifecycle/failure matrix, and #7-to-#25 handoff. The repository now contains an exact-commit native AArch64 PKGBUILD scaffold, strict closed-payload installer, disabled user unit, empty optional-reviewer registry, and archive verifier for payload, owner/mode, dependency, entry-type, and ELF shape. The task installs or enables nothing and labels output non-hermetic and non-publishable. An exact clean image, installed journey, downgrade guard, mixed-version rule, lifecycle/interruption harness, complete SBOM/provenance/signature, and clean-system evidence remain unresolved. |
