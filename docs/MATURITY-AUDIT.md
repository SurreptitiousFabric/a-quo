...
| [#9 Harden signed Omarchy plugin installation and updates](https://github.com/SurreptitiousFabric/a-quo/issues/9) | Omarchy | **Implementing** (was Prototype complete) | Defined | The [install/update/removal prototype](OMARCHY.md) has bounded hostile-archive parsing, staging, and a descriptor-pinned, bounded, no-follow reader for Omarchy's exact version-1 plugin-reference locations in [`a-quo-omarchy`](../crates/a-quo-omarchy/). Raw tar iteration rejects GNU/PAX extension metadata before it can bypass A Quo's 4,096-byte path, 255-byte component, 64-component, and 8,191-entry extracted-tree bounds. Install and removal recheck observed configuration immediately before exposing or quarantining a package directory. Linux fresh install and update open package input no-follow/nonblocking, copy it through a hard byte limit, and use one kernel-sealed artifact for proof verification, archive inspection, extraction, and the package digest recorded in the receipt. Replacing the fresh-install staged pathname after sealing therefore cannot substitute different archive bytes. The bounded update path additionally disables recursive staging cleanup from creation; derives a normalized candidate manifest from those bytes plus the exact local receipt; requires the first descriptor-rooted tree snapshot to match; binds installed manifest/receipt decision bytes to the installed baseline; pins current/candidate trees and both parent directories; exchanges descriptor-relatively; and verifies identities and bounded tree snapshots after rescan. Success retains the verified prior release without automatic purge; rescan or late authorization-finalization failure attempts guarded exact restore and retains the rejected candidate after verified rollback. Mutation, staged-path substitution, semantic-read substitution, `.git` restoration, special/growing input, mode relaxation, rollback, panic, refusal, and finalization fault tests cover the bounded paths. The external Omarchy validator remains a bracketed, explicitly non-continuous pathname observation. Standalone inspection still reopens its caller-supplied path; fresh install does not yet descriptor-pin or snapshot-bind its extracted tree through validation/final move, still has replacement-sensitive automatic staging cleanup, and has no analogous post-operation authorization commit; recovery material is owner-writable after return. The bounded Linux removal path separately pins parent/target/quarantine descriptors, verifies the moved inode, revalidates exact rollback, remains possible after publisher deauthorization, retains quarantine on panic or path races, and deliberately performs no recursive purge. The disconnected Stage-0 record prototype validates asserted pre-operation fields but does not yet observe live configuration or hand records to the installer. A Quo performs no enable action; durable intent/restart recovery, parent-directory `fsync`, descriptor-relative extraction, replacement-safe cleanup, safe purge, complete clean-system corpus execution, external-validator isolation, exact native-report/policy/action handoff, Omarchy-owned load/reference coordination, and external-review gates remain unmet. Behavioural scanning remains Plug & Prejudice responsibility. |
