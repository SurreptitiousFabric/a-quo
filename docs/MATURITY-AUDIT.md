...
| [#7 Safe Omarchy packaging](https://github.com/SurreptitiousFabric/a-quo/issues/7) | Omarchy | Design | Defined | The shared [package and support contract](PACKAGING.md) fixes the three-binary layout, disabled per-user service, busless authority boundary, administrator/user ownership split, one complete pinned Omarchy 4 aarch64 journey, lifecycle/failure matrix, and #7-to-#25 handoff. An exact clean image, package files/unit, downgrade guard, mixed-version rule, install harness, and clean-system evidence remain unresolved, so implementation has not started. |
| [#8 Plugin permissions and runtime risk](https://github.com/SurreptitiousFabric/a-quo/issues/8) | Omarchy | **Design** (was Backlog) | Defined | [Plugin risk evidence](PLUGIN-RISK.md) defines separate exact-byte, publisher, structural, provider-analysis, review, and policy dimensions; an exact sealed regular-file stream; a root-owned provider registry; tagged canonical envelopes; update deltas; explicit unknown/error states; and the Plug & Prejudice boundary. Closed machine-readable schemas, bounded exact-JCS parsers, internal and derivable cross-record checks, a seeded parser fuzz target, and one blocked/indeterminate golden update now cover the publisher/structure/delta/policy/policy-result/assessment record shapes. This is parser/shape/binding work only: provider comparison remains unavailable, provider deltas are digest-bound to the previous/current binding union, provider identity fields are absent beyond provider ID, and permission/coverage claims are rejected until future envelope parsing. Registry/envelope/stream conformance, package scanning, comprehensive hostile vectors and sustained fuzz evidence, concrete isolation limits and threat-decision matrix, adapters/coordinator, capability-policy derivation, eligibility decisions, and the trusted install/update prompt remain unresolved, so the card stays Design / Defined. |
