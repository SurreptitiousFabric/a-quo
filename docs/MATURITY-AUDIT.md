...
| [#9 Harden signed Omarchy plugin installation and updates](https://github.com/SurreptitiousFabric/a-quo/issues/9) | Omarchy | **Implementing** (was Prototype complete) | Defined | The [install/update prototype](OMARCHY.md) has bounded hostile-archive parsing, staging, atomic exchange, rollback tests, and a descriptor-pinned, bounded, no-follow reader for Omarchy's exact version-1 plugin-reference locations in [`a-quo-omarchy`](../crates/a-quo-omarchy/). The installer rechecks observed configuration immediately before package exposure. Separately, the disconnected Stage-0 record prototype validates asserted fields for pre-operation reference state, intent, accepted source, and exact raw-byte digest; it does not yet observe live configuration or hand records to the installer. A Quo performs no enable action, but only Omarchy cooperation through a coordinated transaction or inhibit interface can guarantee that no concurrent reference or transient load races exposure. Crash/power-loss, real-corpus, exact native-report/policy/action handoff, the upstream coordination boundary, and external-review gates remain unmet. Behavioural scanning remains Plug & Prejudice responsibility. |
| [#10 Build a real-world signed Omarchy package and lifecycle test corpus](https://github.com/SurreptitiousFabric/a-quo/issues/10) | Omarchy | **Design** (was Backlog) | Defined | The [pinned corpus baseline](OMARCHY-CORPUS.md) freezes exact public commits for Coin Toss, Omarchy Frame, and Sonarchy as the three representative plugins; a real Frame `0.5.0` → `0.5.1` update family and exact ID-change/equal-version/downgrade refusal cases; and Plug & Prejudice as a separate recursion/integration subject. This A Quo corpus owns package, proof, structure, subject-binding, policy, and lifecycle expectations; behavioural analyzer correctness belongs to Plug & Prejudice. Canonical packages/proofs/streams, the pinned builder and ledger, hostile variants, publication-permission records, retained native-report fixtures, and clean-system results remain absent. |
