...
| [#9 Harden signed Omarchy plugin installation and updates](https://github.com/SurreptitiousFabric/a-quo/issues/9) | Omarchy | **Implementing** (was Prototype complete) | Defined | The [install/update/removal prototype](OMARCHY.md) has bounded hostile-archive parsing, staging, and a descriptor-pinned, bounded, no-follow reader for Omarchy's exact version-1 plugin-reference locations in [`a-quo-omarchy`](../crates/a-quo-omarchy/). Raw tar iteration rejects GNU/PAX extension metadata before it can bypass A Quo's 4,096-byte path, 255-byte component, 64-component, and 8,191-entry extracted-tree bounds. Install and removal recheck observed configuration immediately before exposing or quarantining a package directory. The bounded Linux update path opens package input no-follow/nonblocking and copies it through a hard byte limit; disables recursive staging cleanup from creation; uses one kernel-sealed artifact for proof verification, archive inspection, extraction, and receipt digest; derives a normalized candidate manifest from those bytes plus the exact local receipt; requires the first descriptor-rooted tree snapshot to match; binds installed manifest/receipt decision bytes to the installed baseline; pins current/candidate trees and both parent directories; exchanges descriptor-relatively; and verifies identities and bounded tree snapshots after rescan. Success retains the verified prior release without automatic purge; rescan or late authorization-finalization failure attempts guarded exact restore and retains the rejected candidate after verified rollback. Mutation, staged-path substitution, semantic-read substitution, `.git` restoration, special/growing input, mode relaxation, rollback, panic, refusal, and finalization fault tests cover the bounded path. The external Omarchy validator remains a bracketed, explicitly non-continuous pathname observation. These are update-only point-in-time guarantees: standalone inspection reopens its caller-supplied path, fresh install reopens its staged path, pathname-based candidate extraction does not yet contain intermediate same-user substitution side effects, the analogous fresh-install post-operation authorization commit remains unresolved, and recovery material is owner-writable after return. The bounded Linux removal path separately pins parent/target/quarantine descriptors, verifies the moved inode, revalidates exact rollback, remains possible after publisher deauthorization, retains quarantine on panic or path races, and deliberately performs no recursive purge. The disconnected Stage-0 record prototype validates asserted pre-operation fields but does not yet observe live configuration or hand records to the installer. A Quo performs no enable action; durable intent/restart recovery, parent-directory `fsync`, descriptor-relative extraction, safe purge, real-corpus execution, external-validator isolation, exact native-report/policy/action handoff, Omarchy-owned load/reference coordination, and external-review gates remain unmet. Behavioural scanning remains Plug & Prejudice responsibility. |
| [#10 Build a real-world signed Omarchy package and lifecycle test corpus](https://github.com/SurreptitiousFabric/a-quo/issues/10) | Omarchy | **Implementing** (was Backlog) | Defined | The [pinned corpus baseline](OMARCHY-CORPUS.md) freezes six exact public revisions across Coin Toss, Omarchy Frame, and Sonarchy, including a real Frame update/ID-change/refusal family. A bounded raw-Git-object builder now emits deterministic unsigned packages and exact observation records; two complete same-host builds were byte-identical and their package/tar digests are frozen in the public ledger. This A Quo corpus owns package, proof, structure, subject-binding, policy, and lifecycle expectations; behavioural analyzer correctness belongs to Plug & Prejudice. The package bytes remain local, unsigned, unscanned, and unpublished. Independent reproduction, signed proofs, hostile variants, publication-permission records, retained native-report fixtures, real lifecycle execution, and clean-system results remain absent. |
