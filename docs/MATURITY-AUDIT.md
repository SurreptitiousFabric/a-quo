...
| [#9 Harden signed Omarchy plugin installation and updates](https://github.com/SurreptitiousFabric/a-quo/issues/9) | Omarchy | **Implementing** (was Prototype complete) | Defined | The [install/update/removal prototype](OMARCHY.md) has bounded hostile-archive parsing, staging, atomic exchange, update rollback, and a descriptor-pinned, bounded, no-follow reader for Omarchy's exact version-1 plugin-reference locations in [`a-quo-omarchy`](../crates/a-quo-omarchy/). Install and removal recheck observed configuration immediately before exposing or quarantining a package directory. The bounded Linux removal path pins parent/target/quarantine descriptors, verifies the moved inode, revalidates exact rollback, remains possible after publisher deauthorization, retains quarantine on panic or path races, and deliberately performs no recursive purge. Separately, the disconnected Stage-0 record prototype validates asserted fields for pre-operation reference state, intent, accepted source, and exact raw-byte digest; it does not yet observe live configuration or hand records to the installer. A Quo performs no enable action, but only Omarchy cooperation through a coordinated transaction or inhibit interface can guarantee that no concurrent reference or transient load races exposure or removal. Durable crash recovery, safe purge, real-corpus execution, exact native-report/policy/action handoff, the upstream coordination boundary, and external-review gates remain unmet. Behavioural scanning remains Plug & Prejudice responsibility. |
