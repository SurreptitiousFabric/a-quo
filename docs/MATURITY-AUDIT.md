...
| [#7 Package A Quo safely for Omarchy](https://github.com/SurreptitiousFabric/a-quo/issues/7) | Omarchy | Design | Defined | The shared [package and support contract](PACKAGING.md) fixes the three-binary layout, disabled per-user service, busless authority boundary, administrator/user ownership split, one complete pinned Omarchy 4 aarch64 journey, lifecycle/failure matrix, and #7-to-#25 handoff. An exact clean image, package files/unit, downgrade guard, mixed-version rule, install harness, and clean-system evidence remain unresolved, so implementation has not started. |
...
| [#25 Ship a portable Linux 0.x release](https://github.com/SurreptitiousFabric/a-quo/issues/25) | Platforms | Design | Defined | The Rust workspace, pinned toolchain, [CI](../.github/workflows/ci.yml), [architecture](ARCHITECTURE.md), and shared [package/support contract](PACKAGING.md) are portable foundations. The local scaffold proves only three uninstalled host-native binaries, Rust dependency SBOMs, metadata, and checksums. #7 must first produce the reference package/harness; target qualification, native dependency inventory, signed artifacts, support/update policy, and cross-distribution lifecycle evidence remain open. |
