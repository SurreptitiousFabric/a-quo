...
fail_policy() {
  printf 'isolated lifecycle container policy invariant failed: %s\n' "$1" >&2
  exit 1
}

if [[ "$#" -ne 10 ]]; then
  printf '%s\n' \
    'usage: verify-x86-package-isolated-lifecycle-container-policy.sh MODE INSPECT_JSON CONTAINER_ID IMAGE_ID WORKSPACE TARGET OBSERVER_HOME MISE F1_ROOT SOURCE_COMMIT' >&2
  exit 2
fi
...
readonly EXPECTED_F1_ROOT="$9"
readonly EXPECTED_SOURCE_COMMIT="${10}"
...
for expected_path in \
  ...
  "${EXPECTED_OBSERVER_HOME}" "${EXPECTED_MISE}" "${EXPECTED_F1_ROOT}"; do
  ...
  jq -r \
    ...
    --arg mode "${MODE}" '
      ...
      def exact_mount_inventory:
        ...
        ($c.HostConfig.Mounts | length) == 5 and
        ([$c.HostConfig.Mounts[].Type] | all(. == "bind")) and
        ([$c.HostConfig.Mounts[].Source] | unique | length) == 5 and
        ([$c.HostConfig.Mounts[].Target] | unique | length) == 5 and
        ...
        exact_mount($mise; "/usr/local/bin/mise"; "explicit-read-only") and
      ...
      def exact_runtime_mount_inventory:
        ...
        ($c.Mounts | length) == 5 and
        ([$c.Mounts[].Type] | all(. == "bind")) and
        ([$c.Mounts[].Source] | unique | length) == 5 and
        ([$c.Mounts[].Destination] | unique | length) == 5 and
        ...
        exact_runtime_mount($mise; "/usr/local/bin/mise"; false) and
      ...
      [
        ...
        {name:"entrypoint-command", ok:(
          ...
          $c.Config.Cmd == ["--noprofile", "--norc",
            "/workspace/scripts/run-x86-package-isolated-lifecycle-offline.sh", $commit]
        )},
...
printf 'isolated lifecycle container policy passed exact %s checks\n' "${MODE}"
